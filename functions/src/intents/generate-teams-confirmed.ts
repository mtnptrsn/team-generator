...
export default (request) => agent => {
  console.log('Confirmation', JSON.stringify(request.body))
  ...
  const people = shuffleArray(outputContext.parameters['given-name'].map(name => Object.keys(name).map((key) => name[key])[0]))

  console.log('peopleShuffled', people)
